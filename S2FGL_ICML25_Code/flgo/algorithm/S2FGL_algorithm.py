...
def calculate_fgma_loss(local_features, global_features, top_k=10):
    ...
    def train(self, model):
        ...
        for iter in range(self.num_steps):
            ...
            if hasattr(self, 'codebook'):
                NLIR_loss = federated_knowledge_distillation_loss(node_features, node_features_g, self.codebook,
                                                                temperature=1, lamb=100)
                FGMA_loss = calculate_fgma_loss(sub_features, sub_g, 10)
                loss += 0.05 * NLIR_loss
                loss += FGMA_loss
            loss.backward()
            optimizer.step()
