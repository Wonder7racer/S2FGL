...
import torch
import numpy as np
from flgo.algorithm.S2FGL_utils import (
    select_important_nodes_lis,
    compute_average_lps_test_nodes,
    compute_sis,
...
class BasicParty:
    ...
    def register_objects(self, parties:list, parties_name='pa  rties'):
        r"""
    ...
    def global_test(self, model=None, flag:str='val'):
        ...
        if self.current_round == 1:
            ...
            for client in self.clients:
                client_data = client.train_data.data

                datalist.append(client_data)
                client_data = client_data.to('cuda')
                client.important_nodes, client.important_labels = select_important_nodes_lis(client_data)
                client.important_nodes_k = select_important_nodes_lis_K(client_data)
                client.LPS = compute_average_lps_test_nodes(client_data)
                client.sis = compute_sis(client_data)
            avg_lps = 0
            for client in self.clients:
                avg_lps += client.LPS
            self.sis_list = []
            for client in self.clients:
                self.sis_list.append(client.sis)
