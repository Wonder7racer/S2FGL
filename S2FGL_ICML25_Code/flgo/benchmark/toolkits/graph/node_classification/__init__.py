...
class GeneralCalculator(BasicTaskCalculator):
    ...
    def compute_loss(self, model, model_g, data, adj_low_un, adj_low, adj_high, fr_low_g_list, fr_high_g_list, mlp_list):
        tdata = self.data_to_device(data)
        # 创建一个有效掩码，排除负数标签的样本
        valid_mask = (tdata.y >= 0) & tdata.train_mask

        outputs, node_features = model(tdata, adj_low_un, adj_low, adj_high, fr_high_g_list, fr_low_g_list, mlp_list)
        model_g.eval()
        with torch.no_grad():
            _, node_features_g = model_g(tdata, adj_low_un, adj_low, adj_high, fr_high_g_list, fr_low_g_list, mlp_list)

        # 检查有效样本的数量，避免在没有有效样本时执行无效操作
        if valid_mask.sum().item() == 0:
            return {'loss': torch.tensor(0.0)}  # 如果没有有效样本，返回零损失

        # 使用有效掩码计算损失
        loss = self.criterion(outputs[valid_mask], tdata.y[valid_mask])
        return {'loss': loss}, node_features, node_features_g
    ...
    def test(self, model, dataset, adj_low_un, adj_low, adj_high, batch_size=64, num_workers=0, fr_high_g_list=None, fr_low_g_list=None, mlp_list=None,
        ...
        for batch in loader:
            ...
            if fr_high_g_list is None:
                outputs, node_feature = model(tdata, adj_low_un, adj_low, adj_high)
            else:
                outputs, node_feature = model(tdata, adj_low_un, adj_low, adj_high, fr_high_g_list, fr_low_g_list, mlp_list)

            if valid_mask.sum().item() == 0:
                continue  # 如果有效样本数量为0，跳过这个批次
